...
async def onboard_brain(req: BrainRequest):
    ...
    if req.tier == "endpoint":
        if not req.api_key or not req.model:
            raise HTTPException(400, "endpoint tier requires api_key and model")
        if not req.provider:
            raise HTTPException(400, "endpoint tier requires provider")
...
"""
        ...
        if not req.wallet:
            raise HTTPException(400, "mine tier requires wallet address")
        tier_env = f"""# Generated by hermes-bridge at {time.strftime('%Y-%m-%d %H:%M:%S')}
TIER=mine
INFERENCE_PROVIDER=openai
OPENAI_BASE_URL=https://api.nanogpt.com/v1
